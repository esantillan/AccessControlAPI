...
class Usuario extends \Restserver\Libraries\REST_Controller
{
    ...
    public function token_post ()
    {
        ...
        try{
            ...
            if ($this->form_validation->run('token'))//valido los parámetros
            {
                $params['username'] = sanitize_string($params['username']);
                $params['password'] = sanitize_string($params['password']);

                $user = $this->UsuarioRepository->getUserByUsernameAndPassword($params['username'], $params['password']);//busco al usuario por (nick o email) y contraseña
                
                if(isset($user)){//credenciales válidas
    ...
    public function validateToken_post ()
    {
        ...
        try{
            ...
            if($result['status']){//valido el token
                $response['status'] = parent::HTTP_OK;
                $response['message'] = 'Ok';
            }else{//token inválido
     ...
     * 
     * Parametros: 'resources' "codigo_recurso" o "recurso" (vista v_usuario_opciones)
     */
    public function checkPermission_get ()
    {
        ...
        try{
            $result = $this->authorization_token->validateToken();//compruebo el token

            if($result['status']){
                $params = $this->get();    
                $this->form_validation->set_data($params);
                
                if ($this->form_validation->run('checkPermission'))//compruebo los parámetros
                {
                    $data = $this->authorization_token->userData();
                    $params['resources'] = sanitize_string($params['resources']);
                    $params['system'] = sanitize_string($params['system']);
        
                    $permiso_usuario = $this->UsuarioRepository->getPermissionByUser($data->user, $params['resources'], $params['system']);
                    
                    if(isset($permiso_usuario)){//compruebo que tenga permiso
                        $response['status'] = parent::HTTP_OK;
                        $response['message'] = 'Ok';
                    ...
                    $response['status'] = parent::HTTP_BAD_REQUEST;
                    $response['message'] = isset($params['resources']) ? $this->form_validation->get_errors() : 'No se ha proporcionado el recurso a consultar';
                }
            }else{//token invalido
    ...
    public function getPermissions_get ()
    {
        ...
        try{
            ...
            if($result['status']){//compruebo el token
                $params = $this->get();    
                $this->form_validation->set_data($params);
                
                if ($this->form_validation->run('getPermissions'))//compruebo los parámetros
                {
                    $data = $this->authorization_token->userData();
                    $params['system'] = sanitize_string($params['system']);
        
                    $permisos_usuario = $this->UsuarioRepository->getPermissionsByUser($data->user, $params['system']);
                    
                    $response['status'] = parent::HTTP_OK;
                    if(isset($permisos_usuario)){//compruebo que tenga permiso
                        $response['message'] = 'Ok';
